...
inline void BindWindowToGlobalEvents(IWindow& w) {
    ...
    w.onResize = [](const Resize& r) {
        WindowResizeEvent e(static_cast<uint32_t>(r.width),
                            static_cast<uint32_t>(r.height));
        GlobalEventHandler::Get().Emit(e);
    };
    w.onKey = [](const RawKeyEvent& k) {
        if (k.action == 1) {
            KeyPressedEvent e(k.key, k.scancode, k.mods, false);
            GlobalEventHandler::Get().Emit(e);
        } else if (k.action == 2) {
            KeyPressedEvent e(k.key, k.scancode, k.mods, true);
            GlobalEventHandler::Get().Emit(e);
        } else {
            KeyReleasedEvent e(k.key, k.scancode, k.mods);
            GlobalEventHandler::Get().Emit(e);
        }
    };
    w.onMouseBtn = [](const RawMouseBtn& b) {
        if (b.action) {
            MouseButtonPressedEvent e(b.button); GlobalEventHandler::Get().Emit(e);
        } else {
            MouseButtonReleasedEvent e(b.button); GlobalEventHandler::Get().Emit(e);
        }
    };
    w.onMouseMove = [](const RawMouseMove& m) {
        MouseMovedEvent e(static_cast<float>(m.x), static_cast<float>(m.y));
        GlobalEventHandler::Get().Emit(e);
    };
    w.onMouseWheel = [](const RawMouseWheel& v) {
        MouseScrolledEvent e(static_cast<float>(v.dx), static_cast<float>(v.dy));
        GlobalEventHandler::Get().Emit(e);
    };
}
} // namespace FrameKit
