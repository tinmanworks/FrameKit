...
// Description  :
//      Defines the Log class for the FrameKit framework and client applications.
//      Provides thread-safe logging with console and file outputs, log levels,
...
namespace FrameKit {
    ...
    enum class LogLevel : uint8_t { Trace, Info, Warn, Error, Critical, Off };
    ...
    public:
        ...
        void log(LogLevel lvl, std::string_view fmt, Args&&... args) noexcept {
            if (lvl < m_Level.load(std::memory_order_relaxed) || m_Level.load() == LogLevel::Off)
                return;
