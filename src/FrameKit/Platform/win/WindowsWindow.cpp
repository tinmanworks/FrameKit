 ...
 * License: MIT
 * Description:
 *   GLFW-based Window implementation for Windows with OpenGL version probing.
 */
...
namespace FrameKit {
    ...
    // ---- State shared across windows (lifetime-managed) ----
...
#if defined(FK_DEBUG)
    // WGL/KHR_debug callback (only if debug context + supported)
    static void APIENTRY GLDebugCallback(GLenum source, GLenum type, GLuint id,
        ...
        // Filter noisy notifications if desired (id/severity checks)
        switch (severity) {
    ...
    // Helpers to cast to our strong enums or typedefs safely
    template <typename T>
    ...
    void WindowsWindow::Init(const WindowProps& props) {
        ...
        // Candidate GL versions to try (highest -> lowest)
        ...
        auto tryCreate = [&](int maj, int min)->GLFWwindow* {
            ...
            // On Windows, prefer the native creation API (WGL)
        ...
        // Probe highest -> lowest
        s_Probing = true;
        for (const auto& v : candidates) {
            requestedMaj = v.maj; requestedMin = v.min;
            if ((created = tryCreate(v.maj, v.min))) break;
        }
        s_Probing = false;
...
#if defined(FK_DEBUG)
        if (GLAD_GL_VERSION_4_3) { // core since 4.3
        ...
        FK_CORE_INFO("OpenGL requested: {}.{} via WGL", requestedMaj, requestedMin);
        ...
        // NOW it's legal to set vsync (must have a current context)
        SetVSync(true);
